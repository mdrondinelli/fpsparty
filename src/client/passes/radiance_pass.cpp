...
void Radiance_pass::declare(render_graph::Builder &builder) {
  if (has_camera()) {
    // albedo/depth: written by Gbuffer_pass. sky_view_lut: written by
    // Sky_view_pass, if it ran this frame (its own gating -- camera && sun
    // -- can be true even when this pass's gating -- camera && grid_mesh
    // uploaded -- is, without sun; declaring the read regardless is
