...
auto const grid_create_info = fpsparty::game::Grid_create_info{
  .bounds =
    fpsparty::math::ibox3{
      fpsparty::math::ivec3{0, 0, 0},
      fpsparty::math::ivec3{3, 3, 3},
    },
};
} // namespace

TEST_CASE("Grid raycast returns immediate hit for solid origin cell") {
  auto grid = fpsparty::game::Grid{grid_create_info};
  ...
  auto const hit = grid.raycast(
    ...
    10.0f);
  REQUIRE(hit);
  CHECK(hit->cell_coords == Eigen::Vector3i{1, 1, 1});
...
TEST_CASE("Grid raycast finds axis-aligned hits") {
  ...
  auto const x_hit = grid.raycast(
    ...
    10.0f);
  REQUIRE(x_hit);
  CHECK(x_hit->cell_coords == Eigen::Vector3i{2, 1, 1});
  CHECK(x_hit->normal == Eigen::Vector3i{-1, 0, 0});
  CHECK(x_hit->t == Catch::Approx(1.5f));
  auto const y_hit = grid.raycast(
    {1, 3, 1}, {0.5f, 0.75f, 0.5f}, Eigen::Vector3f{0.0f, -2.0f, 0.0f}, 10.0f);
  REQUIRE(y_hit);
  CHECK(y_hit->cell_coords == Eigen::Vector3i{1, 0, 1});
...
TEST_CASE("Grid raycast steps all tied axes") {
  ...
  auto const hit = grid.raycast(
    {0, 0, 0}, Eigen::Vector3f::Constant(0.5f), {1.0f, 1.0f, 0.0f}, 10.0f);
  REQUIRE(hit);
  CHECK(hit->cell_coords == Eigen::Vector3i{1, 1, 0});
...
TEST_CASE("Grid raycast treats out-of-bounds cells as non-solid") {
  ...
  auto const hit = grid.raycast(
    ...
    10.0f);
  REQUIRE(hit);
  CHECK(hit->cell_coords == Eigen::Vector3i{0, 1, 1});
