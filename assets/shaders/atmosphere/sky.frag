...
float ray_planet(vec3 ro, vec3 rd) {
  const float r_min = r_ground - 400.0f;
  const float b = dot(ro, rd);
...
vec3 integrate_radiance(vec3 x_0, vec3 x_1, vec3 d) {
  ...
  for (int i = 0; i < step_count; ++i) {
    const vec3 x_i = mix(x_0, x_1, (i + 0.5f) / step_count);
...
void main() {
  const vec3 ro =
    vec3(0.0f, r_ground + max(push_constants.altitude, 0.0f), 0.0f);
  const vec3 rd =
    mat3(push_constants.camera_basis) *
    normalize(vec3(vec2(-in_ndc.x, -in_ndc.y) * push_constants.zoom, 1.0f));
  const float t_atmosphere = ray_atmosphere(ro, rd);
  const float t_planet = ray_planet(ro, rd);
  const float t = t_planet >= 0.0f ? t_planet : t_atmosphere;
  const vec3 p = ro + t * rd;
  const bool reverse = altitude(p) < altitude(ro);
  const vec3 x_0 = reverse ? p : ro;
  const vec3 x_1 = reverse ? ro : p;
  const vec3 d = reverse ? -rd : rd;
  const vec3 radiance = integrate_radiance(x_0, x_1, d);
