...
// depths and the depth gradient at p.
float depth_weight(
    vec2 p, vec2 q, float z_p, float z_q, vec2 gradient_z_p) {
  return exp(
    -abs(z_p - z_q) /
    (depth_weight_sigma_z * abs(dot(gradient_z_p, p - q)) +
...
// using the 3x3-gaussian-blurred variance at p
// (distant_irradiance_variance.comp's output) to scale the tolerance.
