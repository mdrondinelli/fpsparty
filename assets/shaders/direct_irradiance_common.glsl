...
const float max_temporal_M = 20.0;
...
// ~10^7 sr^-1 radiance without overflowing fp16 reservoir storage.
// Split into two light-specific functions (rather than one dispatching on
// a stored technique flag) now that sun and sky each have their own
// reservoir: every call site statically knows which light it means.
